...
Data source: nfl_data_py (https://github.com/nflverse/nfl_data_py)
  ...
  - Pulls official nflverse play-by-play data for seasons 2018-2023
"""
...
# ── 1. DATA ───────────────────────────────────────────────────────────────────
print("=" * 60)
print("PHASE 0: Loading NFL Play-by-Play Data via nfl_data_py")
print("=" * 60)
...
try:
    ...
    raw = nfl.import_pbp_data(SEASONS, downcast=True, cache=True)
    print(f"  Raw plays loaded: {len(raw):,}")
    USE_REAL_DATA = True
except ImportError:
    print("  WARNING: nfl_data_py not installed. Run: pip install nfl_data_py")
    print("  Falling back to realistic synthetic data for demonstration.")
    USE_REAL_DATA = False

if USE_REAL_DATA:
    # Keep only standard scrimmage plays (pass + rush), drop special teams noise
    pbp_raw = raw[raw["play_type"].isin(["pass", "run"])].copy()

    # Drop rows missing the fields we need
    required_cols = [
        "game_id", "play_id", "season", "home_team", "away_team",
        "score_differential", "game_seconds_remaining", "yardline_100",
        "down", "ydstogo", "quarter_seconds_remaining",
        "half_seconds_remaining", "qtr", "posteam_score", "defteam_score",
        "home_team_wins",
    ]
    pbp_raw = pbp_raw.dropna(subset=required_cols)

    # home_team_wins: 1 if the home team won, 0 otherwise
    # nflverse encodes this as result > 0 from the home team's perspective
    if "home_team_wins" not in pbp_raw.columns:
        pbp_raw["home_team_wins"] = (pbp_raw["result"] > 0).astype(int)

    pbp = pbp_raw[required_cols].reset_index(drop=True)
    print(f"  After filtering to pass/run plays: {len(pbp):,} plays")
    print(f"  Unique games: {pbp['game_id'].nunique():,}")
    print(f"  Target balance (home win rate): "
          f"{pbp.groupby('game_id')['home_team_wins'].first().mean():.3f}")

else:
    # ── Realistic synthetic fallback ─────────────────────────────────────────
    # This mirrors the statistical distributions of real NFL data but does not
    # require the nfl_data_py dependency. Results will differ from the paper.
    N_GAMES = 4000
    N_PLAYS = 80
...
# ── Game-level train/test split (prevents data leakage) ──────────────────────
# All plays from a single game land in either train or test — never both.
# This ensures the model cannot implicitly learn a game's outcome from adjacent
# plays, which would inflate performance estimates on standard random splits.
...
X_cluster_all = scaler.fit_transform(df[CLUSTER_FEATURES])

X_train = df.loc[train_mask, MODEL_FEATURES].values
X_test  = df.loc[test_mask,  MODEL_FEATURES].values
y_train = df.loc[train_mask, TARGET].values
y_test  = df.loc[test_mask,  TARGET].values
X_cluster_train = X_cluster_all[train_mask.values]
X_cluster_test  = X_cluster_all[test_mask.values]

print(f"  Train plays: {len(X_train):,}  |  Test plays: {len(X_test):,}")
print(f"  Features: {len(MODEL_FEATURES)}  |  Cluster features: {len(CLUSTER_FEATURES)}")
...
for k in K_RANGE:
    ...
    sil_scores.append(silhouette_score(
        X_cluster_train, labels, sample_size=min(5000, len(X_cluster_train)),
        random_state=42))

# k=5 selected: elbow plateaus and silhouette peaks here
BEST_K = 5
...
# IMPORTANT: Identical hyperparameters for cluster and global models.
# Both use max_depth=6, n_estimators=200, min_samples_leaf=10.
# This ensures any performance difference is attributable to the clustering
# step itself, not to a capacity advantage of the cluster-specific models.
RF_PARAMS = dict(
    n_estimators=200,
    max_depth=6,
    min_samples_leaf=10,
...
for c in range(BEST_K):
    ...
    if len(np.unique(ytr)) < 2 or len(Xte) == 0:
        print(f"  Cluster {c}: skipped (insufficient class diversity)")
        continue
    ...
    print(f"  Cluster {c} ({CLUSTER_NAMES[c]}): "
          f"n_train={len(Xtr):,}  acc={cluster_results[c]['accuracy']:.3f}  "
...
# Global RF — same hyperparameters as the cluster models
global_rf = RandomForestClassifier(**RF_PARAMS)
global_rf.fit(X_train, y_train)
global_probs = global_rf.predict_proba(X_test)[:, 1]
global_preds = (global_probs >= 0.5).astype(int)
print(f"  Global RF: acc={accuracy_score(y_test, global_preds):.3f}  "
      f"auc={roc_auc_score(y_test, global_probs):.3f}")
...
print(f"  Pythagorean: acc={accuracy_score(y_test, pyth_preds):.3f}  "
      f"auc={roc_auc_score(y_test, pyth_probs):.3f}")

# ── Hybrid model: stitch together per-cluster predictions ─────────────────────
...
print(f"  Hybrid RF:  acc={accuracy_score(y_test, hybrid_preds):.3f}  "
      f"auc={roc_auc_score(y_test, hybrid_probs):.3f}")
...
for i, met in enumerate(metrics):
    ...
    bars = axes[i].barh(results_df["Model"], vals,
                        color=["#1D9E75" if "Hybrid" in m else "#73726c"
...
fig, ax = plt.subplots(figsize=(8, 6))
ax.barh(top_feats[::-1], top_vals[::-1], color=PALETTE[1])
...
# ── FIG 9: Confusion matrices (Global vs Hybrid) ─────────────────────────────
fig, axes = plt.subplots(1, 2, figsize=(10, 4))
...
outcome = "Home Win" if game_df["home_team_wins"].iloc[0] else "Away Win"
axes[0].set_title(f"In-Game Win Probability Trace\n"
                  f"Game {sample_game} (actual outcome: {outcome})")
axes[0].set_ylim(0, 1)
...
conn.close()
print("\nResults exported to nfl_predictions.db")
